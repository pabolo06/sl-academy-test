...
def get_files():
    ...
    # 1. Source (EXCEPT out, node_modules)
...
def deploy():
    print("Gathering absolute Millennium files...")
    files = get_files()
    ...
    payload = {
        ...
        "params": {
            ...
            "arguments": {
                ...
                "app_name": "SL Academy Millennium",
                "description": "Clean Source Deploy Millennium Edition",
                "frontend_template": "nextjs-static",
                "files": files,
                "intent": "Absolute clean source deploy with minimal config to trigger stable Builder",
    ...
    print("Sending Millennium Deploy...")
    try:
        response = requests.post(ENDPOINT, json=payload, headers=headers, timeout=120)
        print(f"Status: {response.status_code}")
        print(response.text)
    except Exception as e:
        print(f"Millennium failed: {e}")
