-- SL Academy Platform - Row Level Security Policies
...
CREATE POLICY hospitals_select_policy ON hospitals
    FOR SELECT
    USING (id = auth.user_hospital_id() AND deleted_at IS NULL);
...
CREATE POLICY test_attempts_select_manager_policy ON test_attempts
    ...
    USING (
        EXISTS (
            ...
            AND profiles.hospital_id = auth.user_hospital_id()
        )
        AND auth.is_manager()
    );
...
CREATE POLICY doubts_select_policy ON doubts
    ...
    USING (
        ...
        AND (
            ...
            (auth.user_role() = 'doctor' AND profile_id = auth.uid())
...
COMMENT ON FUNCTION auth.user_hospital_id() IS 'Returns the hospital_id of the currently authenticated user';
COMMENT ON FUNCTION auth.user_role() IS 'Returns the role (manager or doctor) of the currently authenticated user';
COMMENT ON FUNCTION auth.is_manager() IS 'Returns true if the currently authenticated user is a manager';
