/**
 * SL Academy Platform - Lesson Management Page
 * Create, edit, reorder, and delete lessons within a track
 */
...
export default function LessonManagementPage() {
  ...
  const fetchTrackAndLessons = async () => {
    try {
      ...
      setTrack(trackData);
      setLessons(lessonsData.filter(l => !l.deleted_at).sort((a, b) => a.order - b.order));
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.order < 0) {
      setError('Ordem deve ser maior ou igual a zero');
      return;
    }

    // Check for duplicate order
    const duplicateOrder = lessons.find(
      l => l.order === formData.order && l.id !== editingLesson?.id
    );
    if (duplicateOrder) {
      setError(`Já existe uma aula com ordem ${formData.order}`);
      return;
    }
    ...
    try {
      ...
      setShowForm(false);
      setFormData({ title: '', description: '', video_url: '', duration_seconds: 0, order: 0 });
      setEditingLesson(null);
      fetchTrackAndLessons();
    } catch (err: any) {
    ...
    <DashboardLayout requiredRole="manager">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          <button
            onClick={() => window.location.href = '/manager/tracks'}
